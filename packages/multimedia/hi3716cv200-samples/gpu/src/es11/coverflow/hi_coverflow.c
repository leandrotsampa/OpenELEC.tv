                                                 ...
                                                 HI_COVERFLOW_S *psCoverFlow)
{
    ...
    psCoverFlow->matrix[psCoverFlow->s32CurrentImageIndex][TARGET_MATRIX].z = -1.0f;
    ...
    for (i = psCoverFlow->s32CurrentImageIndex - 1; i >= 0; --i)
    {
        ...
        psCoverFlow->matrix[i][TARGET_MATRIX].y = 0.0f;

        psCoverFlow->matrix[i][TARGET_MATRIX].z = -1.2f;
        psCoverFlow->matrix[i][TARGET_MATRIX].sizeX = 0.2f;
        psCoverFlow->matrix[i][TARGET_MATRIX].sizeY = 0.8f;

        psCoverFlow->matrix[i][TARGET_MATRIX].rotation = psCoverFlow->angle;
                                      ...
                                      HI_COVERMATRIX_S * Matrix)
{
    ...
    glDrawArrays(GL_TRIANGLE_STRIP, 0, psCoverFlow->hasInvertedImage ? 8 : 4);
