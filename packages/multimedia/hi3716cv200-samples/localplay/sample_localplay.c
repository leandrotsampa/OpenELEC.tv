...
HI_S32 main(HI_S32 argc, HI_CHAR **argv)
{
    ...
    HI_S32 ifsuccess = 0;
#if 0 //spitzbube: No gapless play function
...
#if 0 //spitzbube: No higo (subtitles)
    higoInit();
    ...
    */
    /* CNcomment:¸Ã¶Î´úÂëÓÃÓÚÄ£Äâ´Ódvb ts²¥·ÅÄ£Ê½ÇÐ»»µ½HiPlayer²¥·Å£¬½«Íâ²¿´´½¨µÄavplay¾ä±ú´«µÝ¸ø
       HiPlayer£¬HiPlayerÄÚ²¿²»ÔÙ´´½¨ÐÂµÄavplay¾ä±ú¡£¹Ø±Õ¸Ã¶Î´úÂë£¬s_stParam.hAVPlayer
       ÊôÐÔÖµÉèÖÃÎªHI_SVR_PLAYER_INVALID_HDL£¬HiPlayerÄÚ²¿×Ô¼º´´½¨avplay */
    ...
    /**<Use external created avplay handle to create hiplayer.You must do below things all by yourself.
      ...
      hiplayer->dvb:destroy hiplayer with HI_SVR_PLAYER_Destroy,hiplay may close the av channel, then dvb need to
          reopen it.
    */
    /* CNcomment:Ê¹ÓÃÍâ²¿´´½¨µÄavplay¾ä±ú£¬¸Ãavplay±ØÐëÒÑ¾­°ó¶¨ºÃwindow, soundÉè±¸£¬ÇÒÐè½«windowÎ»ÖÃ¡¢´óÐ¡ÉèÖÃºÃ£¬
       soundÒôÁ¿´óÐ¡ÉèÖÃºÃ£¬HiPlayerÄÚ²¿¶Ôwindow,sound²»×öÈÎºÎ²Ù×÷¡£
       a¡¢dvb -> HiPlayer: Í£Ö¹avplay£¬½«avplay¾ä±ú¸¶¸øs_stParam.hAVPlayer£¬µ÷ÓÃHI_SVR_PLAYER_Create´´½¨HiPlayer£¬
          HiPlayer»á½«avplayÄ£Ê½ÇÐ»»µ½ÏëÒªµÄÄ£Ê½£¬²¢ÇÒ»áÖØÐÂ¿ª¹ØÒôÊÓÆµÍ¨µÀ£»

       b¡¢HiPlayer -> dvb: µ÷ÓÃHI_SVR_PLAYER_Destroy½Ó¿ÚÏú»ÙHiPlayer£¬HiPlayer»á¹Ø±ÕµôÒôÊÓÆµÍ¨µÀ£¬dvbÐèÖØÐÂµ÷ÓÃ
          HI_UNF_AVPLAY_ChnOpen½Ó¿Ú¿ªÆôÒôÊÓÆµÍ¨µÀ£»
    */
    ...
    if (HI_SUCCESS != s32Ret)
    {
        PRINTF("\e[31m ERR: register file parser libprivateprotocol.so fail, ret = 0x%x \e[0m \n", s32Ret);
    }
...
#if 0 //spitzbube: No fast start hls
    /* Fast start hls */
    if (1 == uHlsStartMode)
...
#if 0 //spitzbube: No test protocol
    /* Test invoke */
